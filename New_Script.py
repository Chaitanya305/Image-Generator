...
def python(p_v, base_image):
    ...
    docker_build_command= f'docker build . --build-arg BASE_IMAGE={base_image} --build-arg PYTHON_VERSION={p_v} -t docker-repo/dataflow-base-py{p_v}'
    docker_push_cmd=f'docker push docker-repo/dataflow-base-py{p_v}'
...
def r(r_v, base_image):
    ...
    docker_build_command= f'docker build . --build-arg BASE_IMAGE={base_image} --build-arg R_VERSION={r_v} -t docker-repo/dataflow-base-r{r_v}'
    docker_push_cmd=f'docker push docker-repo/dataflow-base-r{r_v}'
    docker_cmd(cd_command, docker_build_command, docker_push_cmd)

def python_r(p_v, r_v, base_image):
    cd_command=f'cd ./dockerfiles/{base_image}/python_r'
    docker_build_command= f'docker build . --build-arg BASE_IMAGE={base_image} --build-arg R_VERSION={r_v} --build-arg PYTHON_VERSION={p_v} -t docker-repo/dataflow-base-r{r_v}py{p_v}'
    docker_push_cmd=f'docker push docker-repo/dataflow-base-r{r_v}py{p_v}'
    docker_cmd(cd_command, docker_build_command, docker_push_cmd)
